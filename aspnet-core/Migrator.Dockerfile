...
WORKDIR "/src/src/Qna.Game.OnlineServer.DbMigrator"
RUN abp install-libs


FROM build AS publish
RUN dotnet restore
RUN dotnet publish "Qna.Game.OnlineServer.DbMigrator.csproj" -c Release -o /app/publish /p:UseAppHost=false --no-restore -p:RunAnalyzers=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
COPY ["env/out/Migrator/appsettings.json", "appsettings.json"]
ENTRYPOINT ["dotnet", "Qna.Game.OnlineServer.DbMigrator.dll"]
