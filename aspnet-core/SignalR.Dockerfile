...
WORKDIR "/src/src/${ROOT_NAMESPACE}.SignalR.Host"
...
RUN dotnet restore
RUN dotnet publish "${ROOT_NAMESPACE}.SignalR.Host.csproj" -c Release -o /app/publish /p:UseAppHost=false --no-restore -p:RunAnalyzers=false

FROM base AS final

ENV APP_NAME=${ROOT_NAMESPACE}.SignalR.Host.dll
ENV ASPNETCORE_URLS=http://+:80;https://+:443
ENV ASPNETCORE_HTTPS_PORT=44335
ENV ASPNETCORE_Kestrel__Certificates__Default__Password=12345678
ENV ASPNETCORE_Kestrel__Certificates__Default__Path=/https/${ROOT_NAMESPACE}.SignalR.Host.pfx

COPY ["env/certs/default/${ROOT_NAMESPACE}.SignalR.Host.pfx", "/https/${ROOT_NAMESPACE}.SignalR.Host.pfx"]
